...
class JsonRpcBuffer extends EventEmitter {
    ...
    append(data) {
        if (this.index + data.length >= this.buffer.length) {
            this.buffer = Buffer.concat([this.buffer], this.buffer.length + Math.max(DEFAULT_BUFFER_SIZE, data.length));
    ...
    readHeaders() {
        ...
        let i = 0;
        while (i < this.index + 1 && (this.buffer[i] !== CR || this.buffer[i + 1] !== LF || this.buffer[i + 2] !== CR || this.buffer[i + 3] !== LF)) {
            i += 1;
        }
...
class JsonRpcService {
    ...
    write(data) {
        const responseString = JSON.stringify({ jsonrpc: '2.0', ...data }, null, null);

        const responseBuffer = Buffer.from(responseString, 'utf8');
        const headerBuffer = Buffer.from(`Content-Length: ${responseBuffer.length}\r\n\r\n`, 'ascii');
